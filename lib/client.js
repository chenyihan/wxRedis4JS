...
var net = require('net'), buffer = require('buffer'), protocol = require('./protocol.js'), command = require('./Command.js'), responseAdapter = require('./ResponseAdapter'), exception = require('./Exception.js');
const
...
Client.prototype = {
	sendCommand : function(callback, cmd, args) {
		...
		conn.on('data', function(data) {
			...
			try {
				var resp = protocol
						.parseResponse(buffer.Buffer.concat(bufList));
				if (callback) {
					callback(resp, null);
				}
				bufList = [];
		...
		// var newConn = net.connect(this.options.port, this.options.host);
		// self.conn = newConn;
		// sendCommand(callback, cmd, args);
		// });
		// conn.on('data', function(data) {
		// bufList.push(data);
		// try {
