...
function SortedSetParams() {
	if (!(this instanceof SortedSetParams)) {
		return new SortedSetParams();
	}
	this.params = [];
}
SortedSetParams.prototype = {
	weights : function(ws) {
		this.params.push(RedisKeyword.WEIGHTS);
		for (var i = 0; i < ws.length; i++) {
			this.params.push(ws[i]);
		}
		return this;
	},
	aggregate : function(agg) {
		this.params.push(RedisKeyword.AGGREGATE);
		this.params.push(agg);
		return this;
	}
};
...
module.exports.SortedSetParams = SortedSetParams;