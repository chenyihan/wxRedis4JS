...
// }
// if (result == Infinity) {
// result = num1 - Number.MAX_VALUE + Number.MIN_VALUE + num2 - 1;
// } else {
// result = num1 - Number.MIN_VALUE + Number.MAX_VALUE + num2 + 1;
// }
// return result;
// }
...
// }
// var postive = result == Infinity;
// var step = num2 > 0 ? -1 : 1;
// for (var i = num2 + step;; i += step) {
// result = num1 * (num2 - i);
// if (isFinite(result)) {
// if (postive) {
// result = add(result - Number.MAX_VALUE + Number.MIN_VALUE,
// multi(num1, i) - 1);
// } else {
// result = add(result - Number.MIN_VALUE + Number.MAX_VALUE,
// multi(num1, i) - 1);
// }
// return result;
// }
// var isBreak = num2 > 0 ? i <= 0 : i >= 0;
// if (isBreak) {
// break;
// }
// }
...
function objectHashCode(obj) {
	...
	// var h = 0;
	var h = new BigNumber(0);
	for ( var k in obj) {
		var v = obj[k];
		// h += hashCode(k) ^ hashCode(v);
		h = h.add(new BigNumber(hashCode(k) ^ hashCode(v)));
	}
	return h.toString();
}
...
function arrayHashCode(array) {
	...
	// var hashcode = 1;
	var hashcode = new BigNumber(1);
	var p = new BigNumber(31);
	for (var i = 0; i < array.length; i++) {
		var val = array[i];
		// hashCode = 31 * hashcode + hashCode(val);
		hashcode = hashcode.times(p).add(new BigNumber(hashCode(val)));
	}
...
function stringHashCode(str) {
	...
	for (var i = 0; i < str.length; i++) {
		...
		hash = hash.times(p).add(new BigNumber(str.charCodeAt(i)));
	}
...
ConsistHash.prototype = {
	addNode : function(node) {
		var weight = node.weight ? node.weight : this.replicas;
		weight = weight * 160;
		for (var i = 0; i < weight; i++) {
			var key = this.gen_nodekey(node, i);
